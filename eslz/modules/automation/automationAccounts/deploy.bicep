...
module automationAccount_linkedService '../../operationalInsights/workspaces/linkedServices/deploy.bicep' = if (!empty(linkedWorkspaceResourceId)) {
...
module automationAccount_solutions '../../operationsManagement/solutions/deploy.bicep' = [for (gallerySolution, index) in gallerySolutions: if (!empty(linkedWorkspaceResourceId)) {
...
module automationAccount_privateEndpoints '../../network/privateEndpoints/deploy.bicep' = [for (privateEndpoint, index) in privateEndpoints: {

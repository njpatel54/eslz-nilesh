...
module privateLinkScope_scopedResource 'scopedResources/deploy.bicep' = [for (scopedResource, index) in scopedResources: {
  name: '${take(uniqueString(deployment().name, location), 4)}-PvtLinkScope-ScopedRes-${index}'
  params: {
...
module privateLinkScope_privateEndpoints '../../network/privateEndpoints/deploy.bicep' = [for (privateEndpoint, index) in privateEndpoints: {
  name: '${take(uniqueString(deployment().name, location), 4)}-PvtLinkScope-PrivateEndpoint-${index}'
  params: {
    groupIds: [

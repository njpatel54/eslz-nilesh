...
module logAnalyticsWorkspace_dataSources 'dataSources/deploy.bicep' = [for (dataSource, index) in dataSources: {
  ...
  params: {
    ...
    tier: contains(dataSource, 'tier') ? dataSource.tier : 'All'  
    tierSetMethod: contains(dataSource, 'tierSetMethod') ? dataSource.tierSetMethod : 'Custom'  
...
module logAnalyticsWorkspace_solutions '../../operationsManagement/solutions/deploy.bicep' = [for (gallerySolution, index) in gallerySolutions: if (!empty(gallerySolutions)) {
